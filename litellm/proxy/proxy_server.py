...
def showwarning(message, category, filename, lineno, file=None, line=None):
    ...
    # Try to write to the provided file, but fall back to the original
    # warnings.showwarning implementation if writing fails (e.g., closed file)
    if file is not None:
        try:
            # Some file-like objects may be closed during interpreter shutdown
            # or module reloads; guard against that.
            file.write(traceback_info)
            return
        except Exception:
            try:
                # Fallback to original behavior which may handle more cases
                _original_showwarning(message, category, filename, lineno, file, line)
                return
            except Exception:
                # As a last resort, ignore to avoid crashing during import/shutdown
                return
...
# Defer license check instantiation to avoid import-time side-effects (logging/I/O)
# Call get_license_check() to instantiate lazily when needed.


def get_license_check():
    global _license_check
    if _license_check is None:
        try:
...
def is_premium_user() -> bool:
    """Lazy accessor for premium status. Avoids performing I/O at import-time."""
    global premium_user
    # If license check hasn't been instantiated, default to False.
    lc = get_license_check()
    if lc is None:
        premium_user = False
    else:
        try:
            premium_user = bool(lc.is_premium())
        except Exception:
            premium_user = False
    return premium_user


# Defer creating ProxyState/SENSITIVE_DATA_MASKER to avoid side-effects during import
proxy_state = None
...
async def proxy_shutdown_event():
    ...
    try:
        ...
        verbose_proxy_logger.exception("Error disconnecting prisma_client during shutdown")

    try:
        if getattr(litellm, "cache", None) is not None:
            await litellm.cache.disconnect()
    except Exception:
        verbose_proxy_logger.exception("Error disconnecting litellm.cache during shutdown")

    # Close jwt_handler if it exists and has an async close method
    try:
        if "jwt_handler" in globals() and jwt_handler is not None:
            close_fn = getattr(jwt_handler, "close", None)
            if asyncio.iscoroutinefunction(close_fn):
                ...
                try:
                    ...
                    verbose_proxy_logger.exception("Error closing jwt_handler (sync)")
    except Exception:
        verbose_proxy_logger.exception("Error while attempting to close jwt_handler")

    # Close db_writer_client if present
    try:
        if "db_writer_client" in globals() and db_writer_client is not None:
            close_fn = getattr(db_writer_client, "close", None)
            if asyncio.iscoroutinefunction(close_fn):
                await close_fn()
            elif callable(close_fn):
                try:
                    ...
                    verbose_proxy_logger.exception("Error closing db_writer_client (sync)")
    except Exception:
        verbose_proxy_logger.exception("Error while attempting to close db_writer_client")

    # flush remaining langfuse logs
    try:
        if "langfuse" in getattr(litellm, "success_callback", []):
            ...
            if langFuseLogger is not None:
                try:
                    ...
                    verbose_proxy_logger.exception("Error flushing Langfuse logs")
    except Exception:
        # [DO NOT BLOCK shutdown events for this]
        verbose_proxy_logger.exception("Unexpected error during langfuse flush")

    ## RESET CUSTOM VARIABLES ##
    try:
        cleanup_router_config_variables()
    except Exception:
        verbose_proxy_logger.exception("Error cleaning up router config variables during shutdown")
...
model_max_budget_limiter = None
# Register callbacks lazily when the objects are created to avoid import-time side-effects
try:
    if model_max_budget_limiter is not None:
        litellm.logging_callback_manager.add_litellm_callback(model_max_budget_limiter)
except Exception:
    # Do not fail import if registration errors occur
    verbose_proxy_logger.debug("Deferred registration of model_max_budget_limiter failed or skipped")
...
jwt_handler = None  # defer initialization; instantiate when needed
prompt_injection_detection_obj: Optional[_OPTIONAL_PromptInjectionDetection] = None
store_model_in_db: bool = False
open_telemetry_logger: Optional[OpenTelemetry] = None
### INITIALIZE GLOBAL LOGGING OBJECT ###
proxy_logging_obj = None  # create lazily to avoid import-time logging/I/O
### REDIS QUEUE ###
async_result = None
...
class ProxyConfig:
    ...
    async def get_config(self, config_file_path: Optional[str] = None) -> dict:
        ...
        if os.environ.get("LITELLM_CONFIG_BUCKET_NAME") is not None:
            ...
            if bucket_type == "gcs":
                ...
                # get_file_contents_from_s3 is async; ensure it is awaited to avoid un-awaited coroutine warnings
                config = await get_file_contents_from_s3(bucket_name=bucket_name, object_key=object_key)
    ...
    async def load_config(  # noqa: PLR0915
        ...
        if guardrails_v2:
            init_guardrails_v2(all_guardrails=guardrails_v2, config_file_path=config_file_path)
    ...
    def _add_callbacks_from_db_config(self, config_data: dict) -> None:
        ...
        if success_callbacks is not None and isinstance(success_callbacks, list):
            for success_callback in success_callbacks:
                if success_callback in litellm._known_custom_logger_compatible_callbacks:
                    _add_custom_logger_callback_to_specific_event(success_callback, "success")
        ...
        if failure_callbacks is not None and isinstance(failure_callbacks, list):
            for failure_callback in failure_callbacks:
                if failure_callback in litellm._known_custom_logger_compatible_callbacks:
                    _add_custom_logger_callback_to_specific_event(failure_callback, "failure")
                elif failure_callback not in litellm.failure_callback:
                    litellm.logging_callback_manager.add_litellm_failure_callback(failure_callback)
    ...
    def _add_general_settings_from_db_config(
        ...
        if "alerting" in _general_settings:
            if (
                ...
                verbose_proxy_logger.debug("Overriding Default 'alerting' values with db 'alerting' values.")
                general_settings["alerting"] = _general_settings["alerting"]  # override yaml values with db
...
"""
    ...
    try:
            ...
            if llm_model_list is not None and data["model"] in router_model_names:
                for m in llm_model_list:
                    if m["model_name"] == data["model"]:
                        if m["litellm_params"]["model"] in litellm.open_ai_embedding_models or any(
        ...
        verbose_proxy_logger.exception("litellm.proxy.proxy_server.embeddings(): Exception occured - {}".format(str(e)))
        if isinstance(e, HTTPException):
...
async def model_info_v2(
    ...
    for _model in all_models:
        ...
        if litellm_model_info == {}:
            ...
            try:
                # Only call get_model_info if we have a string model name
                if isinstance(litellm_model, str) and litellm_model:
        ...
        if litellm_model_info == {}:
            ...
            if isinstance(litellm_model, str) and litellm_model:
                ...
                litellm_model = None
            try:
                if isinstance(litellm_model, str) and litellm_model and len(split_model) > 0:
        ...
        litellm_params_obj = _model.get("litellm_params")
        if isinstance(litellm_params_obj, dict):
            litellm_params_obj.pop("api_key", None)
            litellm_params_obj.pop("vertex_credentials", None)
            litellm_params_obj.pop("aws_access_key_id", None)
            litellm_params_obj.pop("aws_secret_access_key", None)
            _model["litellm_params"] = litellm_params_obj
...
async def model_info_v1(  # noqa: PLR0915
    ...
    if user_model is not None:
        ...
        _deployment_info_dict = remove_sensitive_info_from_deployment(deployment_dict=_deployment_info_dict)
    ...
    for idx, in_place_model in enumerate(all_models):
        try:
            all_models[idx] = _get_proxy_model_info(model=in_place_model)
        except Exception:
            # If enrichment fails for a model, keep the original and continue
            verbose_proxy_logger.exception("Error enriching model info for model at index %s", idx)
...
async def model_group_info(
    ...
    if (
        ...
        user_object = await prisma_client.db.litellm_usertable.find_first(where={"user_id": user_api_key_dict.user_id})
...
async def login(request: Request):  # noqa: PLR0915
    ...
    if prisma_client is not None:
        _user_row = cast(
            ...
            await prisma_client.db.litellm_usertable.find_first(where={"user_email": {"equals": username}}),
        )
...
async def update_config(config_info: ConfigYAML):  # noqa: PLR0915
    ...
    try:
        ...
        if config_info.litellm_settings is not None:
            ...
            # Merge existing settings with updated settings, giving precedence to updated values
...
async def get_config_general_settings(
    ...
    if db_general_settings is None or db_general_settings.param_value is None:
        ...
        # param_value may be stored as a JSON string or already as a mapping; handle both
        try:
            if isinstance(db_general_settings.param_value, str):
                general_settings = json.loads(db_general_settings.param_value)
            else:
                general_settings = dict(db_general_settings.param_value)
        except Exception:
            raise HTTPException(
                status_code=500,
                detail={"error": "Failed to parse general_settings from DB"},
            )
...
async def get_config_list(
    ...
    if db_general_settings is not None and db_general_settings.param_value is not None:
        try:
            if isinstance(db_general_settings.param_value, str):
                db_general_settings_dict = json.loads(db_general_settings.param_value)
            else:
                db_general_settings_dict = dict(db_general_settings.param_value)
        except Exception:
