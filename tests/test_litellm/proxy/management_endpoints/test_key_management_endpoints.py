...
async def test_budget_reset_at_first_of_month(monkeypatch):
    ...
    assert response_date.year == expected_year, f"Expected year {expected_year}, got {response_date.year}"
    assert response_date.month == expected_month, f"Expected month {expected_month}, got {response_date.month}"
...
async def test_key_update_object_permissions_no_existing_permission(monkeypatch):
    ...
    # Verify find_unique was called once and that it was invoked with a non-None object_permission_id
    mock_prisma_client.db.litellm_objectpermissiontable.find_unique.assert_called_once()
    _, kwargs = mock_prisma_client.db.litellm_objectpermissiontable.find_unique.call_args
    assert "where" in kwargs
    assert kwargs["where"].get("object_permission_id") is not None
