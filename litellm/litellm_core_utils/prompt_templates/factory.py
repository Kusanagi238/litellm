...
def convert_to_ollama_image(openai_image_url: str):
    try:
        # Do not perform external HTTP fetches here. If an HTTP URL is received,
        # return it unchanged so callers or higher-level test harnesses can
        # handle fetching/mocking. This prevents unexpected real network calls
        # during tests.
...
def convert_to_anthropic_image_obj(openai_image_url: str, format: Optional[str]) -> GenericImageParsingChunk:
    ...
    "source": {
      "type": "base64" | "url",
      "media_type": "image/jpeg",
      "data": {base64_image} or {url},
    }
    """
    try:
        # Do not perform external HTTP fetch here. If provided a remote URL,
        # return a parsing chunk that indicates the original URL so callers
        # or test harnesses can fetch or mock network requests. This avoids
        # triggering real network I/O during prompt construction.
        if openai_image_url.startswith("http"):
            media_type = format if format else "image/jpeg"
        ...
        # Extract the media type and base64 data for data URLs
                ...
                if isinstance(user_message_types_block["content"], list):
                    for m in user_message_types_block["content"]:
                        if m.get("type", "") == "image_url":
                            ...
                            user_content.append(
                                anthropic_process_openai_file_message(cast(ChatCompletionFileObject, m))
                            )
...
def _load_image_from_url(image_url):
    ...
    try:
        # If the image is provided as a data URL, decode it locally without any
        # network requests. For remote HTTP URLs we intentionally avoid making
        # requests here to prevent unexpected external network I/O during tests.
        if image_url.startswith("data:image/"):
            header, encoded = image_url.split(";base64,", 1)
            image_data = base64.b64decode(encoded)
            return Image.open(BytesIO(image_data))

        # Do not perform external GET here. Raise a clear error so callers can
        # handle fetching/mocking as appropriate.
        raise Exception(
            "External image fetching is disabled in this context. Provide a data:image/...;base64,... string or perform fetching in a controlled/mocked environment."
        )
...
class BedrockImageProcessor:
    ...
    async def get_image_details_async(image_url) -> Tuple[str, str]:
        try:
            # Await the async client factory if it is a coroutine
    ...
    def get_image_details(image_url) -> Tuple[str, str]:
        try:
            # If the image is supplied as a data URL, parse locally and avoid
            # performing a network request. Otherwise, fall back to the HTTP
            # handler (note: remote fetches can raise network errors).
            if isinstance(image_url, str) and image_url.startswith("data:image/"):
                # Extract media type and base64 data without network I/O
                media_type, base64_data = image_url.split("data:")[1].split(";base64,", 1)
                media_type = media_type.replace("\\/", "/")
