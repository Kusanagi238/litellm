    ...
    try:
        ...
        if kwargs.get("success_callback", None) is not None and isinstance(kwargs["success_callback"], list):
            ...
            for index, callback in enumerate(kwargs["success_callback"]):
                if inspect.iscoroutinefunction(callback) or callback == "dynamodb" or callback == "s3":
        ...
        if (
            ...
            if (
                ...
                rules_obj.pre_call_rules(
                    input="".join(
                        m.get("content", "") for m in messages if "content" in m and isinstance(m["content"], str)
                    ),
                    model=model,
                )
            ...
            file_checksum = litellm.litellm_core_utils.audio_utils.utils.get_audio_file_name(file_obj=_file_obj)
        ...
        try:
            ...
            try:
                verbose_logger.error(
                    "litellm.utils.py::function_setup() - [Non-Blocking] Error in function_setup (logging failed)"
                )
            except Exception:
                # Best-effort logging failed; swallow to avoid masking original exception
                pass
        raise
...
def client(original_function):  # noqa: PLR0915
    ...
    def wrapper(*args, **kwargs):  # noqa: PLR0915
        ...
        if _is_async_request(kwargs):
            ...
            # If the function returned a coroutine in a sync context, run it to completion
            if asyncio.iscoroutine(result):
                try:
                    loop = asyncio.get_event_loop()
                    if loop.is_running():
                        # If an event loop is already running, run in a new loop to avoid 'was never awaited' warnings
                        result = asyncio.run(result)
                    else:
                        result = loop.run_until_complete(result)
                except RuntimeError:
                    # Fallback to asyncio.run if the above fails
                    result = asyncio.run(result)
        ...
        try:
            ...
            # MODEL CALL
            result = original_function(*args, **kwargs)
            # If we received a coroutine in a synchronous wrapper, execute it to avoid 'was never awaited'
            if asyncio.iscoroutine(result):
                try:
                    loop = asyncio.get_event_loop()
                    if loop.is_running():
                        # Running loop present; use asyncio.run to execute in a new event loop
                        result = asyncio.run(result)
                    else:
                        result = loop.run_until_complete(result)
                except RuntimeError:
                ...
                try:
                    ...
                    try:
                        verbose_logger.exception(
                            "litellm.utils.py::client() - logging_obj.failure_handler raised an exception while handling another exception"
                        )
                    except Exception:
                        # swallow logging-on-logging errors to avoid masking the original exception
                        pass
            raise
    ...
    async def wrapper_async(*args, **kwargs):  # noqa: PLR0915
        ...
        try:
            ...
            _caching_handler_response: CachingHandlerResponse = await _llm_caching_handler._async_get_cache(
                model=model or "",
                original_function=original_function,
                logging_obj=logging_obj,
                start_time=start_time,
            ...
            if _is_streaming_request(
                ...
                return result
            ### POST-CALL RULES ###
            post_call_processing(original_response=result, model=model, optional_params=kwargs)
            ...
            if logging_obj:
                try:
                    ...
                    try:
                        verbose_logger.exception(
                            "litellm.utils.py::client() - logging_obj.failure_handler raised an exception during async handling"
                        )
                    except Exception:
                        pass
                try:
                    ...
                    try:
                        verbose_logger.exception(
                            "litellm.utils.py::client() - logging_obj.async_failure_handler raised an exception"
                        )
                    except Exception:
                        pass
...
def update_response_metadata(
    ...
    metadata.set_timing_metrics(start_time=start_time, end_time=end_time, logging_obj=logging_obj)
    metadata.apply()
    ...
    **kwargs,
    ...
    if provider_config is not None:
        ...
        if size is not None:
            ...
            aspect_ratio = size_to_aspect_ratio.get(size, "1:1")  # Default to square if size not recognized
...
def get_optional_params_embeddings(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    ## raise exception if non-default value passed for non-openai/azure embedding calls
        ...
        if model is not None and "text-embedding-3" not in model and "dimensions" in non_default_params.keys():
            raise UnsupportedParamsError(
                status_code=500,
...
class PreProcessNonDefaultParams:
    ...
    def base_pre_process_non_default_params(
        ...
        for k, v in special_params.items():
            # Guard against custom_llm_provider being None/non-string
            is_provider_str = isinstance(custom_llm_provider, str)

            if k.startswith("aws_") and (
                custom_llm_provider != "bedrock"
                and not (is_provider_str and custom_llm_provider.startswith("sagemaker"))
            ):  # allow dynamically setting boto3 init logic
                continue
            elif k == "hf_model_name" and not (is_provider_str and custom_llm_provider == "sagemaker"):
                continue
            elif k.startswith("vertex_") and not (
                is_provider_str and custom_llm_provider in ("vertex_ai", "vertex_ai_beta")
            ):  # allow dynamically setting vertex ai init logic
                continue
            passed_params[k] = v
...
def _get_order_filtered_deployments(healthy_deployments: List[Dict]) -> List:
    ...
    if min_order is not None:
        filtered_deployments = [
            deployment for deployment in healthy_deployments if deployment["litellm_params"].get("order") == min_order
        ]
...
def get_response_string(response_obj: Union[ModelResponse, ModelResponseStream]) -> str:
    _choices: Union[List[Union[Choices, StreamingChoices]], List[StreamingChoices]] = response_obj.choices
...
def _is_potential_model_name_in_model_cost(
    ...
    """
    return any(potential_model_name in litellm.model_cost for potential_model_name in potential_model_names.values())
    ...
    try:
        ...
        if custom_llm_provider == "huggingface":
            ...
            if _model_info is None and model in litellm.model_cost:
                ...
                if not _check_provider_match(model_info=_model_info, custom_llm_provider=custom_llm_provider):
                    _model_info = None
            if _model_info is None and combined_stripped_model_name in litellm.model_cost:
...
def prompt_token_calculator(model, messages):
    ...
    if "claude" in model:
        try:
            ...
            raise ImportError("Anthropic import failed please run `pip install anthropic`")
...
class TextCompletionStreamWrapper:
    ...
    def convert_to_text_completion_object(self, chunk: ModelResponse):
        try:
            ...
            # Ensure chunk is of an expected mapping-like structure with 'choices'
            if not isinstance(chunk, Choices):  # chunk should always be of type StreamingChoices
                raise Exception("Unexpected chunk type while converting to text completion object")
...
async def async_mock_completion_streaming_obj(model_response, mock_response, model, n: Optional[int] = None):
    if isinstance(mock_response, litellm.MockException):
        raise mock_response
    for i in range(0, len(mock_response), 3):
        completion_obj = Delta(role="assistant", content=mock_response[i : i + 3])
...
def add_dummy_tool(custom_llm_provider: str) -> List[ChatCompletionToolParam]:
    ...
    """
    # Try to construct typed objects if available, otherwise fall back to plain dicts
    try:
        from litellm.schemas import ChatCompletionToolParam, ChatCompletionToolParamFunctionChunk  # type: ignore
        ...
        # Fallback to a plain dict structure when the typed classes are not importable
        return [
            {
                "type": "function",
                "function": {
...
def validate_and_fix_openai_messages(messages: List):
    ...
    for message in messages:
        if not message.get("role"):
            # Missing role should default to 'user' to avoid mis-classifying user content as assistant
            message["role"] = "user"
...
class ProviderConfigManager:
    ...
    def get_provider_chat_config(  # noqa: PLR0915
        ...
        if provider == LlmProviders.OPENAI and litellm.openaiOSeriesConfig.is_model_o_series_model(model=model):
        ...
        elif litellm.LlmProviders.AI21 == provider or litellm.LlmProviders.AI21_CHAT == provider:
        ...
        elif litellm.LlmProviders.MISTRAL == provider or litellm.LlmProviders.CODESTRAL == provider:
            return litellm.MistralConfig()
            ...
            elif bedrock_invoke_provider == "meta" or bedrock_invoke_provider == "llama":  # amazon / meta llms
...
def _add_path_to_api_base(api_base: str, ending_path: str) -> str:
    ...
    # Find the maximum overlap between the end of base_segments and the start of end_segments
    max_overlap = 0
    max_k = min(len(base_segments), len(end_segments))
    for k in range(max_k, 0, -1):
        if base_segments[-k:] == end_segments[:k]:
            max_overlap = k
            break

    # Combine segments without duplicating the overlapping portion
    if max_overlap > 0:
        final_segments = base_segments + end_segments[max_overlap:]
    else:
        final_segments = base_segments + end_segments

    # Construct the new path
    modified_path = "/" + "/".join(final_segments) if final_segments else "/"
...
def add_openai_metadata(metadata: dict) -> dict:
    ...
    visible_metadata = {k: v for k, v in metadata.items() if k != "hidden_params" and isinstance(v, (str))}
